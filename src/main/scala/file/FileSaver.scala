...
class FileSaver {
  ...
  def saveFileResults(results: Seq[Result], filename: String): Unit = {
    ...
    val timesOutput = results.map(r => r.statsTracker.getTimeNano.toDouble / 1000000).mkString(" ")
...
//    bw.write("=\n")
  ...
  def saveProgression(result: Result, filename: String): Unit = {
    ...
    val progressionBestOutput = result.statsTracker.getProgressionBest.mkString("", " ", "\n")
    bw.write(progressionOutput)
    bw.write("=\n")
    bw.write(progressionBestOutput)
